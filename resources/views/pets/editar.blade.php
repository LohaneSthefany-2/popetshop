...
<head>
    ...
    <style>
        .bg-custom {
            background-image: url('/images/editarpets.jpg');
...
<body class="min-h-screen bg-custom bg-pink-50/90 bg-blend-overlay antialiased">
    ...
    <div class="p-10 flex justify-center">
        <div class="w-full max-w-xl bg-white/95 backdrop-blur-sm rounded-[2rem] shadow-xl border-4 border-pink-100 p-10">
            <h2 class="text-3xl font-extrabold text-gray-700 drop-shadow-sm mb-8">Editar Pet</h2>

            <form method="POST" action="/pets/{{ $pet->id }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    <label class="text-pink-600 font-bold text-lg block mb-2">Dono</label>
                    <input type="text" name="dono" value="{{ $pet->dono }}"
                        class="w-full p-4 rounded-2xl border-2 border-pink-100 outline-none focus:border-pink-400 bg-pink-50/30 transition-all font-medium text-gray-700 text-lg">
                </div>
                
                <div>
                    <label class="text-pink-600 font-bold text-lg block mb-2">Tipo</label>
